...
<head>
   ...
   <title>Verificação das Tarefas</title>
</head>
<body>
...
<div class="container">
    <div class="header">
        ...
        <div class="for">
            <h1>Central de alterações de Tarefas</h1> 
            <p>
...
$strcon2 = mysqli_connect($servidor2, $usuario2, $senha2, $dbname2) or die ('Erro ao conectar ao banco de dados');


if($parametro){
    $sql = "SELECT * from logsOcorrencia where usuario like '%$parametro%' or visita like '%$parametro%' or descricao like '%$parametro%' order by ID desc";
    ...
    echo "</tr>";
        ...
        $sql = "SELECT * FROM logsOcorrencia order by ID desc";
        $total_registros = "30";
            echo "<table class='tabelaAltera'>";
            echo "<tr>";
            echo "<th>usuários</th>";
        ...
        $todos = mysqli_query($conn2, "$sql");
        $todos2 = mysqli_query($strcon2, "$sql");
        $totalregistros = mysqli_num_rows($todos2);

        $resultado = mysqli_query($strcon2, "$sql LIMIT $inicio,$total_registros") or die ("Erro ao tentar cadastrar registro");
